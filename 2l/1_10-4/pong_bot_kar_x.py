...
if resume:
    model = pickle.load(open('pong_bot_kar_x.p', 'rb'))
else:
    model = {}
...
def policy_forward(x):
    h1 = np.dot(model['W1'], x) # вектор * число -> вектор
    h1[h1 < 0] = 0 # ReLU nonlinearity
    h2 = np.dot(model['W2'], h1) # матрица * вектор -> вектор
    h2[h2 < 0] = 0
    logp = np.dot(model['W3'], h2) # вектор * вектор -> число
    p = sigmoid(logp)
...
def policy_backward(eph1, eph2, epdlogp):
    ...
    dW3 = np.dot(eph2.T, epdlogp).ravel() # матрица * вектор -> вектор

    dh2 = np.outer(epdlogp, model['W3']) # вектор * вектор -> матрица
    dh2[eph2 <= 0] = 0 # backpro prelu
    dW2 = np.dot(dh2.T, eph1) # матрица * матрица -> матрица

    dh1 = np.dot(dh2, model['W2']) # матрица * матрица -> матрица
    dh1[eph1 <= 0] = 0 # backpro prelu
    dW1 = np.dot(dh1.T, epx).ravel() # матрица * вектор -> вектор
...
while True:
    ...
    h1s.append(h1) # first hidden state
    h2s.append(h2) # second hidden state
    ...
    if done: # an episode finished
        ...
        if (episode_number % batch_size == 0):
            ...
            pickle.dump(model, open('pong_bot_kar_x.p', 'wb'))
      ...
      #  reward_sum = 0
        ...
        #print(('ep %d: game finished, re+ward: %f' % (episode_number, reward)) + 
            #  .'' if reward == -1 else ' !!!!!!!!'))
        open('pong_bot_kar_x.csv', 'at').write(str(episode_number) + ',' + str(reward_sum) + ',' + str(running_reward) + '\n'); reward_sum = 0
